...
export async function GET(req: NextRequest) {
  ...
  return NextResponse.json(
    includeDeleted ? items : items.filter((item) => !item.deletedAt),
  );
}
...
export async function POST(req: NextRequest) {
  ...
  await writeAuditLog({
    ...
    action: "create",
    entity: "barang",
    description: `Menambah barang ${newItem.name}`,
  });
...
export async function PUT(req: NextRequest) {
  ...
  await writeAuditLog({
    ...
    action: "update",
    entity: "barang",
    description: `Mengubah barang ${items[idx].name}`,
  });
...
export async function DELETE(req: NextRequest) {
  ...
  const index = items.findIndex(
    (item) => item.id === body.id && !item.deletedAt,
  );
  if (index === -1)
    return NextResponse.json({ success: false }, { status: 404 });
  if (
    items[index].items?.some(
      (serial) => serial.status === 0 || serial.status === 2,
    )
  ) {
    return NextResponse.json(
      { success: false, error: "Barang masih dipinjam atau dibooking" },
      { status: 409 },
    );
  }
  const actor = getActor(req);
  ...
  await writeAuditLog({
    ...
    action: "delete",
    entity: "barang",
    description: `Menghapus barang ${body.id}`,
  });
