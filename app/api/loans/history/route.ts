...
import { isInRecentCalendarWindow, isLoanOperationallyActive, refreshLoanHistory } from "@/lib/loan-history"

const DATABASE_PATH = path.join(process.cwd(), "database")

async function readDatabase<T>(file: string): Promise<T[]> {
  try {
    return JSON.parse(await fs.readFile(path.join(DATABASE_PATH, file), "utf-8")) as T[]
  } catch {
    return []
  }
}
