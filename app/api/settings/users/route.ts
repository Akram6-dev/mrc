...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const newUser = { id: Date.now().toString(), username, password, role: "admin" };
    const updated = { ...settings, users: [...users, newUser] };
    await fs.writeFile(SETTINGS_PATH, JSON.stringify(updated, null, 2), "utf-8");
    const actor = getActor(req);
    await writeAuditLog({ ...actor, action: "create", entity: "akun admin", description: `Menambahkan akun admin ${username}` });
...
export async function PUT(req: NextRequest) {
  ...
  try {
    ...
    const actor = getActor(req);
    await writeAuditLog({ ...actor, action: "update", entity: "akun admin", description: `Mengubah akun admin ${username}` });
    return NextResponse.json({ success: true });
  } catch {
    return NextResponse.json({ error: "Gagal mengubah akun admin" }, { status: 500 });
  }
}
