...
export default function BookPublicPage() {
    ...
    const filteredBorrowers = borrowerSearch.trim() === ""
        ...
        : borrowers.filter((b: any) => {
            ...
            return b.name.toLowerCase().includes(q) || b.nip?.toLowerCase().includes(q) || b.officerId?.toLowerCase().includes(q);
        });
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (!bookingItems.some((it) => it.qty > 0)) throw new Error("Pilih minimal satu barang");
            ...
            for (const it of bookingItems) {
                if (it.qty > it.max) throw new Error(`Jumlah ${it.name} melebihi stok (${it.max})`);
            }
            // Simpan booking
            const res = await fetch("/api/booking", {
                ...
                body: JSON.stringify({
                    ...
                    items: bookingItems.filter((it) => it.qty > 0).map(it => ({ itemId: it.itemId, qty: it.qty })),
    ...
    return (
        <main className="min-h-screen bg-gradient-to-br from-blue-50 to-accent-100 dark:from-gray-900 dark:to-accent-900 p-0 m-0 flex flex-col select-none user-select-none">
            <div className="flex flex-col items-center pt-8 pb-2">
                ...
                />
                <h1 className="text-2xl font-bold text-center tracking-tight text-accent-700 dark:text-accent-200 mb-1">Booking Barang</h1>
            ...
            <form onSubmit={handleSubmit} className="w-full max-w-md mx-auto px-4 pb-8 space-y-5 sm:space-y-6 flex flex-col flex-1">
                    ...
                    <div className="max-h-[420px] overflow-auto rounded-lg border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 divide-y divide-gray-100 dark:divide-gray-800 shadow-sm">
                        {isLoading ? (
                            Array.from({ length: 3 }).map((_, idx) => (
                                <div key={idx} className="flex items-center gap-3 px-2 py-3 sm:py-4 animate-pulse">
                            ...
                            bookingItems.map((item, idx) => {
                                ...
                                const imgUrl = itemData?.image || itemData?.img || "/placeholder.jpg";
                                const desc = itemData?.description || itemData?.desc || "";
                                return (
                                    <div key={item.itemId} className="flex items-center gap-3 px-2 py-3 sm:py-4">
                                        <img
                                            src={imgUrl}
                                            alt={item.name}
                                        ...
                                        <div className="flex-1 min-w-0">
                                            <div className="font-semibold text-base truncate mb-0.5">{item.name}</div>
                                            <div className="text-xs text-gray-500 truncate mb-1.5">{desc}</div>
                                            <div className="text-xs text-accent-700">Stok: {item.max}</div>
                                        </div>
                                        <div className="flex items-center gap-1">
                                            <Button
                                                type="button"
                                                ...
                                                aria-label="Kurangi"
                                                disabled={item.qty <= 0}
                                                onClick={() => setBookingItems(items => items.map((it, i) => i === idx ? { ...it, qty: Math.max(0, it.qty - 1) } : it))}
                                            >
                                                -
                                            </Button>
                                            <Input
                                                ...
                                                onChange={e => {
                                                    ...
                                                    setBookingItems(items => items.map((it, i) => i === idx ? { ...it, qty: val } : it));
                                                }}
                                            ...
                                            <Button
                                                ...
                                                onClick={() => setBookingItems(items => items.map((it, i) => i === idx ? { ...it, qty: Math.min(it.max, it.qty + 1) } : it))}
                                            >
                                                +
                                            </Button>
                                        </div>
                    ...
                    <div className="mt-4 mb-2">
                        {bookingItems.filter(it => it.qty > 0).length > 0 && (
                            <div className="rounded-lg bg-white/90 dark:bg-gray-900/80 border border-gray-200 dark:border-gray-700 shadow-sm px-4 py-3">
                                <div className="font-semibold text-base mb-2 text-accent-700 dark:text-accent-200">Ringkasan Peminjaman</div>
                                {selectedBorrower && (
                                    <div className="mb-2 text-sm text-gray-700 dark:text-gray-300 flex items-center gap-1">
                                        ...
                                        <span className="truncate">{borrowers.find((b: any) => b.id === selectedBorrower)?.name || '-'}</span>
                                    </div>
                                )}
                                <div className="mt-1 text-sm text-gray-700 dark:text-gray-300 flex items-center gap-1">
                                    <span className="font-bold">Barang ({bookingItems.filter(it => it.qty > 0).length}):</span>
                                </div>
                                <ul className="space-y-1">
                                    {bookingItems.filter(it => it.qty > 0).map(it => {
                                        const itemData = items.find((i: any) => i.id === it.itemId);
                                        const idx = bookingItems.filter(x => x.qty > 0).findIndex(x => x.itemId === it.itemId);
                                        return (
                                            <li
                                                key={it.itemId}
                                                className={
                                            ...
                                            >
                                                <span className="truncate flex-1">{itemData?.name || it.name}</span>
                                                <span className="ml-2 text-accent-700 dark:text-accent-200"><span className="font-bold">{it.qty}</span>x</span>
                                            </li>
                                        );
                                    })}
                                </ul>
                ...
                <div className="flex gap-2">
                    <div className="flex-1">
                        <Label className="block text-base font-semibold mb-1">Tanggal Pinjam <span className="text-red-500">*</span></Label>
                        <DatePickerField
                            value={startDate}
                    ...
                    <div className="w-32">
                        <Label className="block text-base font-semibold mb-2">Durasi (hari) <span className="text-red-500">*</span></Label>
                ...
                <div>
                    <Label className="block text-base font-semibold mb-2">Jam Pengambilan <span className="text-red-500">*</span></Label>
                    <Input
                        type="time"
                        value={pickupTime}
                ...
                <div>
                    <Label className="block text-base font-semibold mb-2">Keperluan <span className="text-red-500">*</span></Label>
                    <Input
                        type="text"
                        value={purpose}
                ...
                <Dialog open={showSuccessDialog}>
                    <DialogContent className="max-w-xs rounded-xl">
                        <DialogHeader>
                            <DialogTitle className="text-green-600 text-center">Booking Berhasil!</DialogTitle>
                        </DialogHeader>
                        <div className="text-center text-base mt-2 mb-2">
                            Silakan datang ke MRC pada jam <span className="font-bold">{pickupTime}</span> sesuai yang dipilih.<br />
