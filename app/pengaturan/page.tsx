...
export default function PengaturanPage() {
  ...
  const handleProfileSubmit = async (e: React.FormEvent) => {
    ...
    if (!profileData.currentPassword) {
      setError(
        "Password saat ini wajib diisi untuk mengubah profil atau password admin.",
      );
      return;
    }
      ...
      const updated = {
        ...
        admin: {
          ...
          password: profileData.newPassword
            ? profileData.newPassword
            : settings.admin?.password || "admin123",
        },
      };
  ...
  const handleAdminAccountSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const newSettings = await fetch("/api/settings").then((response) =>
        response.json(),
      );
      setSettings(newSettings);
  ...
  const handleDeleteAdmin = async (id: string, username: string) => {
    ...
    setSettings(
      await fetch("/api/settings").then((response) => response.json()),
    );
  ...
  const handleExportExcel = async () => {
    try {
      ...
      XLSX.utils.book_append_sheet(
        workbook,
        XLSX.utils.json_to_sheet(toRows(items)),
        "items",
      );
      XLSX.utils.book_append_sheet(
        workbook,
        XLSX.utils.json_to_sheet(toRows(borrowers)),
        "borrowers",
      );
      XLSX.utils.book_append_sheet(
        workbook,
        XLSX.utils.json_to_sheet(toRows(loans)),
        "loans",
      );
      XLSX.writeFile(
        workbook,
        `mrc-backup-${new Date().toISOString().split("T")[0]}.xlsx`,
      );
  ...
  const parseExcelSheet = (workbook: XLSX.WorkBook, sheetName: string) => {
    const actualSheetName = workbook.SheetNames.find(
      (name) => name.toLowerCase() === sheetName,
    );
    if (!actualSheetName) return null;
  ...
  const parseNestedItems = (records: Record<string, unknown>[]) =>
    records.map((record) => {
      ...
      try {
        ...
        throw new Error("Kolom items harus berisi JSON yang valid");
      }
    });
  ...
  const handleImportFile = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!file) return;
    try {
      let data: { items: unknown[]; borrowers: unknown[]; loans: unknown[] };
      if (
        file.name.toLowerCase().endsWith(".xlsx") ||
        file.name.toLowerCase().endsWith(".xls")
      ) {
        const workbook = XLSX.read(await file.arrayBuffer(), { type: "array" });
        const items = parseExcelSheet(workbook, "items");
        const borrowers = parseExcelSheet(workbook, "borrowers");
        const loans = parseExcelSheet(workbook, "loans");
        if (!items || !borrowers || !loans) {
          setError(
            "Format Excel tidak valid. Workbook harus memiliki sheet items, borrowers, dan loans.",
          );
          return;
        }
        data = {
          items: parseNestedItems(items),
          borrowers,
          loans: parseNestedItems(loans),
        };
      } else {
      ...
      if (!data.items || !data.borrowers || !data.loans) {
        setError(
          "Format file tidak valid. Pastikan file backup dari aplikasi ini.",
        );
        return;
      }
      ...
      if (responses.some((response) => !response.ok))
        throw new Error("Salah satu data gagal diimpor");
      setSuccess("Data berhasil diimpor dan database ditimpa.");
      // Optionally reload settings or data
      // window.location.reload();
    } catch (err) {
      setError(
        "Gagal mengimpor data. Format file tidak valid atau terjadi kesalahan.",
      );
    }
  };
  ...
  useEffect(() => {
    const poll = async () => {
      try {
        ...
        if (
          data.length > 0 &&
          data[0].id !== lastNotifIdRef.current &&
          !data[0].read
        ) {
                      ...
                      {[
                        {
                          key: "aiReply",
                          label: "Balasan AI",
                          desc: "Aktifkan balasan otomatis dari AI.",
                        },
                        {
                          key: "loanMessage",
                          label: "Kirim Pesan Peminjaman",
                          desc: "Aktifkan pesan saat peminjaman.",
                        },
                        {
                          key: "returnMessage",
                          label: "Kirim Pesan Pengembalian",
                          desc: "Aktifkan pesan saat pengembalian.",
                        },
                        {
                          key: "reminderMessage",
                          label: "Kirim Pesan Pengingat",
                          desc: "Aktifkan tombol pesan pengingat.",
                        },
                      ].map((item) => (
                        <div
                          key={item.key}
                  ...
                  <div className="space-y-6">
                    <div className="p-6 bg-blue-50 dark:bg-blue-900/20 rounded-xl border border-blue-200 dark:border-blue-800">
                      ...
                      <button
                        ...
                        className="flex items-center px-5 py-2 rounded-lg font-medium bg-blue-600 text-white hover:bg-blue-700 focus:ring-2 focus:ring-blue-400 transition-colors shadow-sm"
                      >
                      ...
                      <button
                        ...
                        className="mt-3 flex items-center px-5 py-2 rounded-lg font-medium bg-emerald-600 text-white hover:bg-emerald-700 focus:ring-2 focus:ring-emerald-400 transition-colors shadow-sm"
                      >
                        <Download className="w-4 h-4 mr-2" />
                        Ekspor Excel
                      </button>
                    ...
                    <div className="p-6 bg-yellow-50 dark:bg-yellow-900/20 rounded-xl border border-yellow-200 dark:border-yellow-800">
                      ...
                      <p className="text-sm text-yellow-700 dark:text-yellow-400 mb-4">
                        Impor data dari file backup JSON atau Excel. File Excel
                        wajib memiliki sheet items, borrowers, dan loans.
                      </p>
                      <Input
                        type="file"
                      ...
                      <label
                        htmlFor="import-file"
                        className="flex w-max items-center px-5 py-2 rounded-lg font-medium bg-yellow-600 text-white hover:bg-yellow-700 focus:ring-2 focus:ring-yellow-400 transition-colors shadow-sm cursor-pointer"
                      >
                    ...
                    <div className="p-6 bg-red-50 dark:bg-red-900/20 rounded-xl border border-red-200 dark:border-red-800">
                      ...
                      <p className="text-sm text-red-700 dark:text-red-400 mb-4">
                        Hapus semua data aplikasi. Tindakan ini tidak dapat
                        dibatalkan!
                      </p>
                      <button
                        onClick={async () => {
                ...
                currentUser.role === "super_admin" && (
                  <div>
                    ...
                    <p className="text-sm text-gray-600 dark:text-gray-400 mb-6">
                      Tambahkan akun petugas yang dapat mengelola peminjaman,
                      pengembalian, dan booking.
                    </p>
                    <form
                    ...
                    <div className="mt-8 border-t border-gray-200 dark:border-gray-700 pt-6">
                      ...
                      <div className="space-y-2">
                        {(settings?.users || [])
                          ...
                          .map((user: any) => (
                            <div
                              key={user.id}
                              className="flex items-center justify-between rounded-lg bg-gray-50 dark:bg-gray-700/50 px-4 py-3"
                            >
                              ...
                              <div className="flex items-center gap-2">
                                ...
                                <button
                                  ...
                                  title="Hapus akun"
                                  onClick={() =>
                                    handleDeleteAdmin(user.id, user.username)
                                  }
                      ...
                      {editingAdminId && (
                        <button
                          ...
                          className="mt-3 text-sm text-gray-500 hover:text-gray-800 flex items-center gap-1"
                        >
