...
export function isInRecentCalendarWindow(dateValue: string, now = new Date()): boolean {
  const date = new Date(dateValue)
  const cutoff = getThreeMonthCutoff(now)
  return date.getUTCFullYear() === now.getUTCFullYear() && date.getTime() >= cutoff.getTime()
}
...
export function isLoanOperationallyActive(loan: Loan, items: Item[]): boolean {
  ...
  return items.some((item) =>
    item.items?.some((serial) => serial.loanId === loan.id && serial.status === 0),
  )
}
...
export function buildRecentLoanHistory(loans: Loan[], items: Item[]): Loan[] {
  ...
  return [...uniqueLoans.values()].sort(
    (a, b) => new Date(b.createdAt || b.borrowDate).getTime() - new Date(a.createdAt || a.borrowDate).getTime(),
  )
}
...
export async function refreshLoanHistory(loans?: Loan[], items?: Item[]): Promise<Loan[]> {
  const sourceLoans = loans ?? JSON.parse(await fs.readFile(LOANS_PATH, "utf-8")) as Loan[]
  const sourceItems = items ?? JSON.parse(await fs.readFile(ITEMS_PATH, "utf-8")) as Item[]
